...
function downloadPDF() {
  ...
  // Capture a top-aligned clone, not the live card. Capturing the live node
  // makes html2pdf retain its scroll position and can leave most of page one
  // blank when the result is lower down the app.
  const exportMount = document.createElement("div");
  const exportContent = pdfContent.cloneNode(true);
  exportMount.className = "pdf-export-mount";
  exportContent.removeAttribute("id");
  exportContent.classList.add("pdf-exporting");
  exportContent.setAttribute("aria-hidden", "true");
  exportMount.appendChild(exportContent);
  ...
  const options = {
    ...
    pagebreak: {
      // Avoiding every element caused html2pdf to move the opening section to
      // a later page, leaving a mostly blank first page. Keep rows intact but
      // allow normal document flow between sections.
      mode: ["css", "legacy"],
      avoid: ["tr", ".pdf-keep-together"]
    }
  };
  ...
  waitForFonts
    .then(() => html2pdf().set(options).from(exportContent).save())
    .then(() => {
